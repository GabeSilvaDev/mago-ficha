...
class FichaPdf {
  ...
  static Future<Uint8List> gerar(Ficha f) async {
    ...
    ///
    /// Precisa ser refeito por passada: as páginas oficiais são pintadas por
    /// `foregroundPainter`, que só roda quando o documento é serializado — e o
    /// pacote `pdf` não deixa acrescentar página nem salvar de novo depois.
    Future<Uint8List> montar(List<ItemAnexo> anexo) async {
      ...
      pagina(_png2!, (g, fonte) {
        ...
        if (quals.length > qY.length) {
          anexoGerado.add(ItemAnexo('Qualidades (continuação)',
              quals.skip(qY.length).map((q) => '${q[0]}: ${q[1]} pts').join('\n')));
        }
        ...
        if (defs.length > dY.length) {
          anexoGerado.add(ItemAnexo('Defeitos (continuação)',
              defs.skip(dY.length).map((d) => '${d[0]}: ${d[1]} pts').join('\n')));
        }
        // Blocos de texto
    ...
    // 1ª passada: desenha e, de quebra, descobre o que foi cortado.
    anexoGerado.clear();
    final semAnexo = await montar(const []);

    // O que não depende do desenho entra depois, na ordem final do anexo.
    final specs = <String>[
      for (final e in GameData.esferas)
        if (f.especEsferaDe(e.chave).isNotEmpty)
          '${e.nome}: ${f.especEsferaDe(e.chave).join(', ')}'
    ...
    anexoGerado.clear();
    final comAnexo = await montar(itens);
    anexoGerado
      ..clear()
      ..addAll(itens);
    return comAnexo;
